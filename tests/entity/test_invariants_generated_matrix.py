"""generate_magic_square() 결과가 INV-* 불변식을 만족하는지 검증."""
...
def test_inv_size_generated_matrix_is_4x4(
    ...
    assert len(generated_matrix) == 4
    assert all(len(row) == 4 for row in generated_matrix)
...
def test_inv_positive_natural_generated_matrix_cells_are_at_least_one(
    ...
    assert all(cell >= 1 for cell in flat_cells(generated_matrix))
...
def test_inv_range_3digit_generated_matrix_cells_are_within_999(
    ...
    assert all(1 <= cell <= MAX_CELL_VALUE for cell in flat_cells(generated_matrix))
...
def test_inv_row_sum_generated_matrix_each_row_sums_to_magic_constant(
    ...
    assert row_sums(generated_matrix) == [MAGIC_CONSTANT] * 4
...
def test_inv_col_sum_generated_matrix_each_column_sums_to_magic_constant(
    ...
    assert col_sums(generated_matrix) == [MAGIC_CONSTANT] * 4
...
def test_inv_diag_sum_generated_matrix_diagonals_sum_to_magic_constant(
    ...
    assert main_diag == MAGIC_CONSTANT
    assert anti_diag == MAGIC_CONSTANT
